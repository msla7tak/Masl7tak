...
public interface ServicesRepository extends JpaRepository<Services, Long> {
    ...
    @Query("SELECT DISTINCT new com.application.masl7tak.dto.ServicesDTO(S.id, S.discountValue,S.images, S.creationDate, S.validUntil, S.rate, S.category.id, " +
            ...
            "AND (:searchKey is null OR P.name  LIKE %:searchKey% OR C.name  LIKE %:searchKey%) " +
...
//            "s.category = COALESCE(:category, s.category), " +
            ...
            "WHERE s.id = :id")
    void update(Long id,String images, double discountValue, Long carBrand, Long carModel, int maxUsage, String validUntil, String is_available);
    @Modifying
